...
namespace CppCLRWinFormsProject {
    ...
    public:
        ...
        // ИСПРАВЛЕНО: Сверяемся с латинскими системными именами типов аккаунтов
        static void RefreshStats(List<Client^>^ clients) {
            ...
            for each(Client ^ cl in clients) {
                for each(BankAccount ^ acc in cl->Accounts) {
                    ...
                    else if (type == "savings") savingsCount++;
                    else if (type == "credit") creditCount++;
                }
            }
            ...
            if (chartPanel != nullptr) chartPanel->Invalidate();
        }
